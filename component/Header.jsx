...
const Header = () => {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 bg-transparent">
      <div
        className={`flex justify-between items-center px-8 py-6 ${textColor}`}
      >
        {/* Logo */}
        <Link href="/">
      ...
      <nav
        className={`absolute top-12 right-0 w-35 md:w-44 bg-transparent h-50 transform transition-transform duration-300 ease-in-out translate-x-0`}
      >
        <div className="flex flex-col w-fit items-center justify-center h-full">
          <ul
            className={`flex flex-col gap-4 text-2xl tracking-wide text-start ${textColor}`}
          >
            <Link href="/">
              <li
                className={`group cursor-pointer transition-all duration-300 text-[16px] ${
                  isMenuOpen
              ...
              >
                <span className="relative inline-block">
                  Home
                  <span
                    className={`absolute bottom-0 left-0 w-0 h-0.5 ${underline} transition-all duration-300 group-hover:w-full`}
                  ></span>
                </span>
              </li>
            </Link>
